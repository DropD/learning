...
class NotesToFretted:
    ...
    @visit.register
    def visit_start(self, node: note_graph.Start) -> Iterator[StartNode]:
        yield StartNode(next=[])

    @visit.register
    def visit_end(self, node: note_graph.End) -> Iterator[EndNode]:
        yield EndNode(prev=[])

    @visit.register
    def visit_note(self, node: note_fragment.Note) -> Iterator[FrettedNode]:
    ...
    @visit.register
    def visit_rest(self, node: note_fragment.Rest) -> Iterator[RestNode]:
...
def build_from_notes(notes: note_graph.Node) -> StartNode:
    if not isinstance(notes.value, note_graph.Start):
        raise ValueError(
            "Parameter 'notes' must be a start node. Use NotesToFretted directly to build tab graph from fragments."
        )
    root = next(NotesToFretted().visit(notes))
    assert isinstance(root, StartNode)
    return root
...
class TxtTab:
    ...
    @functools.singledispatchmethod
    def visit(
        self, node: Node, *, sequence: list[list[str]]
    ) -> Iterator[list[list[str]]]:
        yield sequence

    @visit.register
    def visit_start(
        self, node: StartNode | MiddleNode, *, sequence: list[list[str]]
    ) -> Iterator[list[list[str]]]:
    ...
    @visit.register
    def visit_fretted(
        self, node: FrettedNode, *, sequence: list[list[str]]
    ) -> Iterator[list[list[str]]]:
    ...
    @visit.register
    def visit_rest(
        self, node: RestNode, *, sequence: list[list[str]]
    ) -> Iterator[list[list[str]]]:
...
def txt_tab_all(tree: StartNode, *, tabber: TxtTab = TxtTab()) -> None:
